...
RUN if [[ "$MVND_VERSION" = "latest" ]]; then curl -fsSL -o mvnd.zip `wget -qO- -t1 -T2 "https://api.github.com/repos/apache/maven-mvnd/releases/latest" | grep "browser_download_url" | grep 'linux-amd64.zip"' | head -n 1 | awk -F ': "' '{print $2}' | sed 's/\"//g;s/,//g;s/ //g'` ; else curl -fsSL -o mvnd.zip ${MVND_DOWNLOAD_URL} ; fi
